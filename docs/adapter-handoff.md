...
`df-web-review-kit`를 host repo 안에서 검증한 뒤 별도 package repo로 옮기기 위한 handoff 문서다.
...
- Host wiring: consuming project's `/review` route
...
- 이 RLS는 dev 검증용이다.
- production host에서는 Supabase Edge Function 또는 project-scoped backend proxy가 더 안전하다.
...
6. package repo에서 `pnpm typecheck`
7. package repo에서 `pnpm build`
8. host repo에서 review route typecheck/build

## Open questions

- Supabase는 optional adapter/sample로 유지한다.
