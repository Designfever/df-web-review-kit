...
- QA note/area composer에서 paste attachment와 iframe viewport capture를 같은 attachment queue로 다룬다.
...
- DOM/note/area draft는 attachment만 있어도 제출할 수 있다.
...
- source locator component hint는 TypeScript가 host dependency로 있을 때만 동작한다. 없으면 기존 file/line/column hint만 사용한다.
...
- adapter boundary 문서에 QA attachment와 Figma image store 책임 분리를 반영했다.
...
- Source Tree가 `/`, `/components/`, `/long-form/` fixture에서 depth 있는 hierarchy로 보이는지 확인했다.
