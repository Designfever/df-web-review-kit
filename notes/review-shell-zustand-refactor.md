...
- [x] **Session D — container extraction for remaining features.** Landed in `2fc0a62`. Sessions A–C moved state but `shell.tsx` still instantiates feature hooks and renders wide JSX prop blocks (SectionOutlinePanel ~25 props, Topbar ~20). This session applies the QaPanelContainer pattern without moving any more state: (1) add a `ReviewShellRefsContext` created in the `ReviewShell` wrapper so containers can reach `iframeRef`/`controllerRef`/`frameScrollRef` etc. without prop-threading; (2) `SectionOutlineContainer` owns `useReviewSectionOutline` + panel JSX (the explicit `refreshCurrentSectionOutline` calls in `toggleSourceTreePanel`/`loadTargetFrame` are removed — the hook's own effect on `isPanelVisible`/`targetFrameLoadVersion` already covers both); (3) `TopbarContainer` owns address/viewport/copy-URL wiring, reading target slice state directly. Out of scope with reasons: `FigmaImagesPanel` (its hook state is consumed by topbar + target frame + panel simultaneously — containerizing would just relay ~18 props; proper fix is a figma-images slice, revisit if the feature grows) and `ReviewTargetFrame` (dominated by ruler hook outputs that hotkeys/topbar also consume).
