...
export function useReviewSectionOutline({
  ...
  const resetSectionOutline = useCallback(() => {
    onClearSourceInspector();
    onClearSourceSelection();
  ...
  const applySectionOutline = useCallback(
    (nextSectionOutline: SectionOutlineEntry[], resetCollapse: boolean) => {
  ...
  const { getCurrentSectionOutline, refreshCurrentSectionOutline } = useOutlineObservation({
    iframeRef,
    sectionOutlineOptions,
    isPanelVisible,
    targetFrameLoadVersion,
    targetSrc,
    onReset: resetSectionOutline,
    onRefresh: applySectionOutline,
  });
